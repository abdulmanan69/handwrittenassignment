...
function setCSSVariable(variable, value) {
  ...
  // Page size controls
  function getSizePx(name) {
    // 96 DPI approximations in px
    const map = {
      A4: [794, 1123],       // 8.27 x 11.69 in
      A5: [559, 794],        // 5.83 x 8.27 in
      Letter: [816, 1056],   // 8.5 x 11 in
      Legal: [816, 1344]     // 8.5 x 14 in
    };
    return map[name] || map.A4;
  }
  function applyPageSizeFromControls() {
    const sizeSel = document.getElementById('page-size-select');
    const orientSel = document.getElementById('page-orientation-select');
    const size = sizeSel ? sizeSel.value : 'A4';
    const orientation = orientSel ? orientSel.value : 'portrait';
    let [w, h] = getSizePx(size);
  ...
  window.addEventListener('DOMContentLoaded', () => {
    applyPageSizeFromControls();
    const sizeSel = document.getElementById('page-size-select');
    const orientSel = document.getElementById('page-orientation-select');
    if (sizeSel) sizeSel.addEventListener('change', applyPageSizeFromControls);
    if (orientSel) orientSel.addEventListener('change', applyPageSizeFromControls);
  });
...
// Floating mini toolbar for heading/left-margin editing
(function(){
  const toolbar = document.getElementById('mini-toolbar');
  const topMargin = document.getElementById('top-margin');
  const leftMarginIn = document.getElementById('left-margin-in');
  const fontSelect = document.getElementById('mini-font-family');
  const sizeSelect = document.getElementById('mini-font-size');
  const colorInput = document.getElementById('mini-color');
  const boldBtn = document.getElementById('mini-bold');
  const italicBtn = document.getElementById('mini-italic');
  const underlineBtn = document.getElementById('mini-underline');

  let lastRange = null;
  let currentWrapper = null; // span used to apply styles persistently
  function selectionInsideEditable() {
    const sel = window.getSelection();
    if (!sel || sel.rangeCount === 0) return null;
    const range = sel.getRangeAt(0);
    let node = range.commonAncestorContainer;
    if (node && node.nodeType === 3) node = node.parentNode; // normalize to element
    if ((topMargin && topMargin.contains(node)) || (leftMarginIn && leftMarginIn.contains(node))) {
      return range;
    }
    return null;
  }
  function saveSelection() {
    const r = selectionInsideEditable();
    if (r && !r.collapsed) {
  ...
  function positionToolbar(range) {
    ...
    const left = (rect.left + rect.right)/2 + window.scrollX - tb.offsetWidth/2;
  ...
  function getOrCreateWrapper() {
    // If we already have a wrapper and selection is within it, reuse it
    if (currentWrapper && currentWrapper.isConnected) return currentWrapper;
    if (!restoreSelection()) return null;
    const sel = window.getSelection();
    if (!sel.rangeCount) return null;
    const range = sel.getRangeAt(0);
    if (range.collapsed) return null;
    // Create a wrapper span for persistent formatting
    const span = document.createElement('span');
    span.setAttribute('data-mini-format', '1');
    // Ensure baseline styles that prevent dimming
    ...
    // Move selection inside wrapper for subsequent edits
  ...
  function applyToWrapper(styleObj) {
    const span = getOrCreateWrapper();
    if (!span) return;
    for (const [k, v] of Object.entries(styleObj)) {
      const kebab = k.startsWith('--') ? k : toKebab(k);
      try { span.style.setProperty(kebab, v, 'important'); } catch {}
      try { span.style[k] = v; } catch {}
      // special-case webkit text fill
      if (k.toLowerCase() === 'webkittextfillcolor' || k === '-webkit-text-fill-color') {
        try { span.style.setProperty('-webkit-text-fill-color', v, 'important'); } catch {}
        try { span.style.webkitTextFillColor = v; } catch {}
      }
    }
    saveSelection();
  }

  // Show/hide + position on selection events
  ['mouseup','keyup'].forEach(evt => {
    document.addEventListener(evt, (e) => {
      ...
      if (r && !r.collapsed) {
        lastRange = r.cloneRange();
        // Update currentWrapper to nearest existing wrapper in selection
        let node = r.commonAncestorContainer.nodeType === 3 ? r.commonAncestorContainer.parentNode : r.commonAncestorContainer;
        currentWrapper = node.closest ? node.closest('[data-mini-format="1"]') : null;
  ...
  window.addEventListener('scroll', () => { if (lastRange) positionToolbar(lastRange); }, true);
  window.addEventListener('resize', () => { if (lastRange) positionToolbar(lastRange); });

  // Only prevent default for buttons to keep selection; allow selects/color to open
  // Keep selection active when using toolbar controls
  ...
  // Also restore on focus for selects/color to allow drag without losing selection
  [fontSelect, sizeSelect, colorInput].forEach(el => {
    if (!el) return;
    el.addEventListener('focus', restoreSelection);
    el.addEventListener('mousedown', restoreSelection);
  });
  ...
  // Actions (apply directly to persistent wrapper)
  function cssFontStack(name){
    return `${name}, CustomFont, AnotherFont, KaTeX_Main, sans-serif`;
  }
  function applyFontFamilyNow(){
    const name = fontSelect.value;
    try { restoreSelection(); document.execCommand('fontName', false, name); } catch {}
    applyToWrapper({ 'font-family': cssFontStack(name) });
  }
  function applyFontSizeNow(){
    const size = sizeSelect.value; // e.g., '16px'
    applyToWrapper({ 'font-size': size, 'line-height': 'normal' });
  }
  ['change','input'].forEach(evt=>{
    fontSelect.addEventListener(evt, applyFontFamilyNow);
    sizeSelect.addEventListener(evt, applyFontSizeNow);
  });
  function selectionRootElement() {
    const sel = window.getSelection();
    if (!sel || sel.rangeCount === 0) return null;
    const range = sel.getRangeAt(0);
    let node = range.commonAncestorContainer;
    if (node.nodeType === 3) node = node.parentNode;
    // limit to top-margin or left-margin-in
    if (topMargin && topMargin.contains(node)) return topMargin;
    if (leftMarginIn && leftMarginIn.contains(node)) return leftMarginIn;
    return null;
  }
  function normalizeFontTags(root, color) {
    if (!root) return;
  ...
  function applyInlineColor(hex) {
    // Apply to persistent wrapper so dragging updates live
    applyToWrapper({
      color: hex,
      WebkitTextFillColor: hex,
      mixBlendMode: 'normal',
      filter: 'none',
      opacity: '1'
    });
  ...
  colorInput.addEventListener('input', () => {
    applyInlineColor(colorInput.value);
  });
  function toggleStyle(prop, onValue, offValue) {
    const span = getOrCreateWrapper();
    if (!span) return;
    const cur = (span.style.getPropertyValue(prop) || '').trim();
    const next = cur === onValue ? (offValue || '') : onValue;
    if (next) span.style.setProperty(prop, next, 'important'); else span.style.removeProperty(prop);
    saveSelection();
  }
  boldBtn.addEventListener('click', () => { toggleStyle('font-weight', 'bold'); });
  italicBtn.addEventListener('click', () => { toggleStyle('font-style', 'italic'); });
  underlineBtn.addEventListener('click', () => { toggleStyle('text-decoration', 'underline'); });

  // Hide when clicking outside editable and toolbar
  document.addEventListener('mousedown', (e) => {
    if (!toolbar.contains(e.target) && !topMargin.contains(e.target) && !leftMarginIn.contains(e.target)) {
      hideToolbar();
      lastRange = null;
    }
  });
...
async function generatePDFfromPages() {
    ...
    const orientSelect = document.getElementById('page-orientation-select');
    const format = (sizeSelect && sizeSelect.value ? sizeSelect.value : 'A4').toLowerCase();
    const orientation = (orientSelect && orientSelect.value ? orientSelect.value : 'portrait');
...
let isDrawingMode = false; // Drawing panel hidden by default
        function toggleMode() {
            ...
            const isHidden = drawingContainer.style.display === '' || drawingContainer.style.display === 'none';
            if (isHidden) {
                drawingContainer.style.display = 'block';
                toggleButton.innerText = 'Close Drawing';
                adjustCanvasToContainer();
            } else {
                drawingContainer.style.display = 'none';
                toggleButton.innerText = 'Open Drawing';
            }
